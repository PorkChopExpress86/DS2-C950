...
def packages_to_csv():
    """
    This will convert the package Excel file into a csv file called package.csv
    """
    ...
    for row in ws.iter_rows(min_row=9, max_row=48, min_col=2, max_col=8):
        package_address = []
        for cell in row:
            cell_str = str(cell.value).replace('\r', '').replace('\n', '').replace("South ", "S").replace(
                "North ", "N").replace("East ", "E").replace("West ", "W")
...
def addresses_to_csv():
    ...
    for row in ws.iter_rows(min_row=9, max_row=35, min_col=1, max_col=1):
        address = []
        for cell in row:
            cell_list = (str(cell.value).replace('\r', '').replace(',', '').replace("South ", "S").replace(
                "North ", "N").replace("East ", "E").replace("West ", "W")).split('\n')
